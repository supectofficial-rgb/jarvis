namespace Insurance.HubService.Endpoints.Api.Extensions.Db;

using Insurance.HubService.Infra.Persistence.RDB.Commands;
using Insurance.HubService.Infra.Persistence.RDB.Queries;
...
public static class DbExtensions
{
    public static IServiceCollection AddDatabase(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddDbContext<HubServiceCommandDbContext>(options =>
        {
            options.UseNpgsql(commandDb);
            AppContext.SetSwitch("Npgsql.EnableLegacyTimestampBehavior", true);
        });

        services.AddDbContext<HubServiceQueryDbContext>(options =>
        {
