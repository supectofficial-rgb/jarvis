namespace Insurance.InventoryService.AppCore.AppServices.Reservations.Commands.ExpireReservation;
...
public class ExpireReservationCommandHandler : CommandHandler<ExpireReservationCommand, Guid>
{
    private readonly IInventoryReservationCommandRepository _repository;

    public ExpireReservationCommandHandler(IInventoryReservationCommandRepository repository)
    {
        _repository = repository;
    }

    public override async Task<CommandResult<Guid>> Handle(ExpireReservationCommand command)
    {
        ...
        try
        {
            reservation.Expire(command.ReasonCode);
        }
        catch (Exception ex)
        {
