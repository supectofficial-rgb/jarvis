...
public sealed class InventoryDocumentsController : Controller
{
    ...
    public InventoryDocumentsController(IDashboardConfigService dashboardConfigService)
    {
    ...
    [HttpGet]
    public async Task<IActionResult> Documents(CancellationToken cancellationToken = default)
    {
        ...
        var model = new DashboardViewModel
        {
            ...
            Roles = roles,
            Modules = modules,
            ActiveModule = modules.FirstOrDefault(x => string.Equals(x.ModuleId, "inventory_documents", StringComparison.OrdinalIgnoreCase)),
        };
        model.ActiveItem = model.ActiveModule?.Items.FirstOrDefault(x => string.Equals(x.ItemId, "documents", StringComparison.OrdinalIgnoreCase));
