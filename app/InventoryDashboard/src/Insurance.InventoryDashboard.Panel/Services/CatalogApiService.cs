...
public sealed class CatalogApiService : ICatalogApiService
{
    ...
    public Task<ApiResponse<List<CategoryVariantNameFormulaModel>>> GetCategoryVariantNameFormulasAsync(string categoryId, string token, bool includeInactive = true) => _apiService.GetCategoryVariantNameFormulasAsync(categoryId, token, includeInactive);
    public Task<ApiResponse<CategoryVariantNameFormulaModel>> GetCategoryVariantNameFormulaByBusinessKeyAsync(string formulaId, string token) => _apiService.GetCategoryVariantNameFormulaByBusinessKeyAsync(formulaId, token);
    ...
    public Task<ApiResponse<bool>> CreateCategoryVariantNameFormulaAsync(string categoryId, UpsertCategoryVariantNameFormulaRequest request, string token) => _apiService.CreateCategoryVariantNameFormulaAsync(categoryId, request, token);
    public Task<ApiResponse<bool>> UpdateCategoryVariantNameFormulaAsync(string formulaId, UpsertCategoryVariantNameFormulaRequest request, string token) => _apiService.UpdateCategoryVariantNameFormulaAsync(formulaId, request, token);
    public Task<ApiResponse<bool>> DeleteCategoryVariantNameFormulaAsync(string formulaId, string token) => _apiService.DeleteCategoryVariantNameFormulaAsync(formulaId, token);
    ...
    public Task<ApiResponse<bool>> UpsertVariantComponentAsync(string variantId, UpsertVariantComponentRequest request, string token) => _apiService.UpsertVariantComponentAsync(variantId, request, token);
    public Task<ApiResponse<bool>> RemoveVariantComponentAsync(string variantId, string componentVariantId, string? componentId, string token) => _apiService.RemoveVariantComponentAsync(variantId, componentVariantId, componentId, token);
    public Task<ApiResponse<List<VariantComponentModel>>> GetVariantComponentsByVariantIdAsync(string variantId, string token) => _apiService.GetVariantComponentsByVariantIdAsync(variantId, token);
    ...
    public Task<ApiResponse<bool>> RemoveVariantAddOnAsync(string variantId, string addOnVariantId, string token) => _apiService.RemoveVariantAddOnAsync(variantId, addOnVariantId, token);
    ...
    public Task<ApiResponse<bool>> RemoveVariantImageAsync(string variantId, string fileKey, string token) => _apiService.RemoveVariantImageAsync(variantId, fileKey, token);
    ...
    public Task<ApiResponse<bool>> UpsertVariantTagAsync(string variantId, UpsertVariantTagRequest request, string token) => _apiService.UpsertVariantTagAsync(variantId, request, token);
    public Task<ApiResponse<bool>> RemoveVariantTagAsync(string variantId, string? variantTagBusinessKey, string? tagName, string token) => _apiService.RemoveVariantTagAsync(variantId, variantTagBusinessKey, tagName, token);
    ...
    public Task<ApiResponse<ProductVariantSearchResultModel>> SearchProductVariantsAsync(string token, string? searchTerm = null, string? productId = null, string? categoryId = null, string? attributeOptionIds = null, bool? isActive = null, int page = 1, int pageSize = 10) => _apiService.SearchProductVariantsAsync(token, searchTerm, productId, categoryId, attributeOptionIds, isActive, page, pageSize);
    ...
    public Task<ApiResponse<StockDetailSearchResultModel>> SearchStockDetailsAsync(string token, string? variantId = null, bool? isEmpty = null, int page = 1, int pageSize = 200) => _apiService.SearchStockDetailsAsync(token, variantId: variantId, isEmpty: isEmpty, page: page, pageSize: pageSize);
    public Task<ApiResponse<List<WarehouseLookupItemModel>>> GetWarehouseLookupAsync(string token, bool includeInactive = true) => _apiService.GetWarehouseLookupAsync(token, includeInactive);
    public Task<ApiResponse<List<SellerLookupItemModel>>> GetSellerLookupAsync(string token, bool includeInactive = true) => _apiService.GetSellerLookupAsync(token, includeInactive);
    public Task<ApiResponse<List<LocationLookupItemModel>>> GetLocationLookupAsync(string token, string? warehouseId = null, bool includeInactive = false) => _apiService.GetLocationLookupAsync(token, warehouseId, includeInactive);
    public Task<ApiResponse<List<QualityStatusLookupItemModel>>> GetQualityStatusLookupAsync(string token, bool includeInactive = false) => _apiService.GetQualityStatusLookupAsync(token, includeInactive);
    public Task<ApiResponse<List<InventoryTransactionListItemModel>>> GetInventoryTransactionsByVariantAsync(string variantId, string token) => _apiService.GetInventoryTransactionsByVariantAsync(variantId, token);
    public Task<ApiResponse<SellerVariantPriceSearchResultModel>> SearchSellerVariantPricesAsync(string token, Guid? sellerRef = null, Guid? variantRef = null, Guid? priceTypeRef = null, Guid? priceChannelRef = null, bool? isActive = null, int page = 1, int pageSize = 50) => _apiService.SearchSellerVariantPricesAsync(token, sellerRef, variantRef, priceTypeRef, priceChannelRef, isActive, page, pageSize);
    public Task<ApiResponse<StockDetailBucketResultModel>> GetAvailableStockBucketsAsync(string token, Guid? variantRef = null, Guid? sellerRef = null, decimal minQuantity = 0) => _apiService.GetAvailableStockBucketsAsync(token, variantRef, sellerRef, minQuantity);
